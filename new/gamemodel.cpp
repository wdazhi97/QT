    ...
    foreach( const QSerialPortInfo &Info,QSerialPortInfo::availablePorts())//读取串口信息
        {
            ...
            my_serialPort->setPortName(port);
            my_serialPort->open( QIODevice::ReadWrite );
            qDebug() << "COM";
            my_serialPort->setBaudRate(QSerialPort::Baud9600);//波特率
...
void GameModel::actionByAI(int &clickRow, int &clickCol)
{
    ...
    seriportwrite(clickRow,clickCol);
}
...
void GameModel::seriportwrite(int xpos,int ypos)
{
   QString ttr=QObject::tr("CJ(%1,%2)").arg(xpos).arg(ypos);
    my_serialPort->write(ttr.toUtf8() );
}
